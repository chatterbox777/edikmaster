...
export default function Select({ labelId, labelText, carsList, countries }) {
  ...
  function handleSelect(e) {
    switch (labelId) {
      case 'maker_selector':
        dispatch(carsActionCreators.setChosenMaker({ chosenMaker: e.target.value }));
        break;

      case 'model_selector':
        dispatch(carsActionCreators.setChosenModel({ chosenModel: e.target.value }));
        break;

      case 'country_selector': {
        let value = countries.find(el => el.name === e.target.value)?.code ?? '';
  ...
  useEffect(() => {
    switch (labelId) {
      ...
      case 'model_selector':
        if (chosenMaker) {
          let currentBrandObject = carsList.find(el => el.brand === chosenMaker);
